...
app.post('/pesquisar', (req, res) => {
  ...
  //const noticias = PostagensController.buscarNoticias(req.query.titulo)
  Postagem.findOne({titulo: new RegExp(req.body.search)}).then((items) => {
      res.end(JSON.stringify(items));
    }).catch((err) => {
      console.log('ta dando erro o livesearch')
    }) 

});

app.get('/conteudo', (req, res) => {
  console.log('Recarregar noticias');
  Postagem.find({titulo: new RegExp(req.body.search)}).then((items) => {
    res.end(JSON.stringify(items));
  }).catch((err) => {
    console.log('deu erro ao carregar noticias')
  }) 
});
